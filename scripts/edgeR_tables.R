...
### Usage example:	Rscript edgeR_tables.R . pick_me ./subsets
#######
...
# Remove version numbers from gene identifiers
subsets_gene <- lapply(subsets_gene, function(set) sub('\\.\\d+$', "", set))
# Load 'objects' object from first file
load(files[1])
# Get unambiguous exon identifiers
u_ids <- rownames(objects$dge_exon)
# Get ambiguous exon identifiers (that match with gene identifiers)
a_ids <- sub('\\.\\d+$', "", u_ids)
## Apply over each element of gene subset list and extract exon identifier subsets
subsets_exon <- lapply(subsets_gene, function(set) u_ids[which(a_ids %in% set)])
...
rm(objects, subsets_file, in_folder, pattern, u_ids, a_ids)
###
