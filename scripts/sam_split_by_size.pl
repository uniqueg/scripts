...
OPTIONS:
   ...
   --prefix STRING    Prefix for output files. The suffix "XXX" will be appended
                      to the prefix, where XXX is the read length [default:     
                      basename of first processed input file if reading from    
                      file(s) or "out_file.sam" if reading from STDIN].
...
sub main {
	...
	#---> BODY <---#	
		...
		while (my $line = <>) {
			...
			#---> Split line, extract read length, skip if not in requested range <---#
			...
			unless ($report_only) {
				...
				# Build filename
				my $file = $prefix . $read_length_formatted;
