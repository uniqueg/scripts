...
param_gene_trx_no_bins_max <- c(2, 5, 8, 12)
...
## Load lookup tables
lookup_trx_2_gene <- read.table(file.path(root, "resources", "gencode.v19.annotation.ENS_compatible.filtered.trx_gene_lookup_table"), col.names=c("trx_id", "gene_id"), colClasses=c("character", "character"))
lookup_trx_2_pas  <- read.table(file.path(root, "resources", "gencode.v19.annotation.ENS_compatible.filtered.trx_pas_lookup_table"), col.names=c("trx_id", "pas_id"), colClasses=c("character", "character"))
lookup_pas_2_gene <- read.table(file.path(root, "resources", "gencode.v19.annotation.ENS_compatible.filtered.pas_gene_lookup_table"), col.names=c("pas_id", "gene_id"), colClasses=c("character", "character"))
...
pas_df_1 <- read.table(file.path(root, "estimates", "hsa_1.processing_sites.A-seq-2.reference"), col.names=c("pas_id", "value"), colClasses=c("character", "numeric"), row.names=1)
pas_df_2 <- read.table(file.path(root, "estimates", "hsa_1.processing_sites.A-seq-2.reference"), col.names=c("pas_id", "value"), colClasses=c("character", "numeric"), row.names=1)
