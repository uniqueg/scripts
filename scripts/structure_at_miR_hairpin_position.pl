...
#---> BODY <---#
	...
	# Print structures
	&print_substructures($dot_bracket_hash_ref, $pos_AoA_ref, $output, $unpaired, $fr_unpaired, $loop_size, $loop_overlap, $dist_loop);
...
sub miRNA_hairpin_positions_to_AoA {
	...
	#---> BODY <---#
		...
		while (<POS>) {
			chomp;
			my @values = split /\t/;
			my $short_name = shift @values;
			$short_name = (split / /)[0];
			unshift @values, $short_name;
...
sub print_substructures {
	...
	#---> BODY <---#
		...
		foreach my $pos_array_ref (@pos_AoA) {
			...
			my ($short_name, $motif_start, $motif_end, $score, $seq) = @$pos_array_ref;
			...
			my $out_line = join "\t", $short_name, $motif_start, $motif_end, $score, $seq, $substructure;

			#---> Check for extra output options <---#			

				#---> I. Number and fraction of unpaired bases <---#
				if ($unpaired || $fr_unpaired) {
					...
					$out_line .= "\t" . $unpaired_number if $unpaired;
					$out_line .= "\t" . $unpaired_fraction if $fr_unpaired;
	
				}

				#---> II. Loop size, overlap and distance from loop <---#
				if ($loop_size || $loop_overlap || $dist_loop) {
					...
					#---> Append requested information to output string <---#
					$out_line .= "\t" . $size if $loop_size;
					$out_line .= "\t" . $overlap if $loop_overlap;
					$out_line .= "\t" . $distance if $dist_loop;
