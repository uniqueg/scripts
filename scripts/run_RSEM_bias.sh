...
rsem_command="rsem_calculate_expression --sam --strand-specific --no-qualities --seed-length $rsem_seed_length --fragment-length-mean $rsem_fragment_length_mean --fragment-length-sd $rsem_fragment_length_sd -p $threads --estimate-rspd --time $sam_trx_rsem $rsem_index_files_prefix $rsem_output_files_prefix"
...
cut -f 1,7 $tmp/RSEM_results*.isoforms.results | tail -n +2 > $estimates_dir/RSEM_bias.estimates
cut -f 1,7 $tmp/RSEM_results*.genes.results | tail -n +2 > $estimates_dir/RSEM_bias.estimates.gene
