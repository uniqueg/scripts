...
col_select <- 2
select <- c("BitSeq", "BitSeq_no_priors", "CEM", "Cufflinks", "eXpress", "IsoEM", "MMSEQ", "MMSEQ_no_priors", "RSEM", "rSeq", "Sailfish", "Scripture", "TIGAR2", "Counting_Transcript", "Counting_Union_exon")
prefix_false_pos <- file.path(root, "plots", "sim.gene.false_pos_vs_read_depth.all")
prefix_false_neg <- file.path(root, "plots", "sim.gene.false_neg_vs_read_depth.all")
prefix_true_pos <- file.path(root, "plots", "sim.gene.true_pos_vs_read_depth.all")
prefix_true_neg <- file.path(root, "plots", "sim.gene.true_neg_vs_read_depth.all")
...
ls_false_pos <- lapply(read_depths, function(read_depth) {
        df <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".gene.false_pos_rate.tab")), sep="\t")[ , col_select, drop=FALSE]
	colnames(df) <- paste("sim", read_depth, sep="_")
	return(df)
})
ls_false_neg <- lapply(read_depths, function(read_depth) {
        df <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".gene.false_neg_rate.tab")), sep="\t")[ , col_select, drop=FALSE]
        colnames(df) <- paste("sim", read_depth, sep="_")
        return(df)
})
ls_true_pos <- lapply(read_depths, function(read_depth) {
        df <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".gene.true_pos_rate.tab")), sep="\t")[ , col_select, drop=FALSE]
        colnames(df) <- paste("sim", read_depth, sep="_")
        return(df)
})
ls_true_neg <- lapply(read_depths, function(read_depth) {
        df <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".gene.true_neg_rate.tab")), sep="\t")[ , col_select, drop=FALSE]
        colnames(df) <- paste("sim", read_depth, sep="_")
        return(df)
})
...
## PLOT
plot_false_pos_depth(df_false_pos, prefix_false_pos, 0.0, 1.0)
plot_false_neg_depth(df_false_neg, prefix_false_neg, 0.0, 1.0)
plot_true_pos_depth(df_true_pos, prefix_true_pos, 0.0, 1.0)
plot_true_neg_depth(df_true_neg, prefix_true_neg, 0.0, 1.0)
