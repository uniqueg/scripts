...
# Convert AsiSI sites to IRanges and add name
tmp <- asi_local$name
asi_local <- ranges(asi_local)
names(asi_local) <- tmp
# Determine local maximum Z score
...
g_track <- GenomeAxisTrack(range=asi_local, fill.range="brown", col.range="brown", showId=TRUE, cex.id=1.2, col.id=NA)
