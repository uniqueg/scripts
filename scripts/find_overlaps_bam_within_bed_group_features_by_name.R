...
### Usage example: ./Rscript ./find_overlaps_bam_within_bed_group_features_by_name.R ./feature_file.bed /path/to/BAM/files/ hg19 /path/to/OUT/files/count_table  
...
# Group by gene
features_grl <- split(features_gr, features_gr$name)
...
dump <- lapply(read_files, function(current_bam) {
	...
	hits <- findOverlaps(reads_gr, features_grl, type="within")
	...
	dimnames(count_table) <- list(make.unique(names(features_grl)), bam_basename)
	# Save count table
