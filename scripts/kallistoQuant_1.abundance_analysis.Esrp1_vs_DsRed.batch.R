...
# Sample/group names
names <- list()
names$reference <- "DsRed"
names$query <- "Esrp1"
names$comparison <- paste(names$query, names$reference, sep="_vs_")

# Output directory
outDir <- file.path("analyzedData/kallistoQuant_1.analysis", names$comparison)
dir.create(outDir, showWarnings = FALSE)

# Input filenames
files <- list()
files$abundances <- "analyzedData/kallistoQuant_1/kallistoQuant.tpm"
...
cols <- list()
cols$reference <- c(1, 4, 7, 10, 13, 16, 19, 22, 25)
cols$query <- c(1, 3, 6, 9, 12, 15, 18, 21, 24)
cols$commonRef <- 1
...
axisLabels$xMarks <- c(0,1,2,3,4,5,6,7,8)
axisLabels$xMarksAlt <- c(1,2,3,4,5,6,7,8)
