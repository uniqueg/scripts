...
## DENSITIES
all_gene_den_ls <- apply(all_gene_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_all_gene_den_ls <- apply(ref_all_gene_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_spl_gene_den_ls <- apply(ref_spl_gene_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_3_gene_den_ls <- apply(ref_3_gene_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))

all_exon_den_ls <- apply(all_exon_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_all_exon_den_ls <- apply(ref_all_exon_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_spl_exon_den_ls <- apply(ref_spl_exon_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_3_exon_den_ls <- apply(ref_3_exon_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))

all_splice_den_ls <- apply(all_splice_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_all_splice_den_ls <- apply(ref_all_splice_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_spl_splice_den_ls <- apply(ref_spl_splice_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))
ref_3_splice_den_ls <- apply(ref_3_splice_fc, 2, function(col) density(col, from=min(col[is.finite(col)]), to=max(col[is.finite(col)])))

## DENSITIES FDR < CUTOFF
all_gene_den_cut_ls <- lapply(all_gene_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_all_gene_den_cut_ls <- lapply(ref_all_gene_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_spl_gene_den_cut_ls <- lapply(ref_spl_gene_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_3_gene_den_cut_ls <- lapply(ref_3_gene_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))

all_exon_den_cut_ls <- lapply(all_exon_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_all_exon_den_cut_ls <- lapply(ref_all_exon_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_spl_exon_den_cut_ls <- lapply(ref_spl_exon_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_3_exon_den_cut_ls <- lapply(ref_3_exon_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))

all_splice_den_cut_ls <- lapply(all_splice_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_all_splice_den_cut_ls <- lapply(ref_all_splice_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_spl_splice_den_cut_ls <- lapply(ref_spl_splice_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
ref_3_splice_den_cut_ls <- lapply(ref_3_splice_fc_cut_ls, function(elem) density(elem, from=min(elem[is.finite(elem)]), to=max(elem[is.finite(elem)])))
###
...
# ref splice factors
pdf("gene_splice.pdf")
plot(0, xlab="log2 fold change", ylab="density", xlim=c(-8, 8), ylim=c(0, 1.5))
...
pdf("gene_3.pdf")
plot(-1, xlim=c(-8, 8), ylim=c(0, 1.5))
