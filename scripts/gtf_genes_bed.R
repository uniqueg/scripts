...
#---> DESCRIPTION <---#
description <- "Converts the gene entries of a GTF file to a BED file with one line per gene.\n"
author <- "Author: Alexander Kanitz, Biozentrum, University of Basel"
version <- "Version: 1.0.0 (29-FEB-2016)"
...
option_list <- list(
		...
		make_option(c("-n", "--name"), action="store", type="character", default="gene_id", help="Attribute to be used for the 'name' (fourth) column in the BED output file (check GTF file for available options; default: 'gene_id').", metavar="string"),
...
#---> FILTER EXONS <---#
# Print status message
if ( opt$verbose ) cat("Filtering genes...\n")
...
# Print status message
if ( opt$verbose ) cat("Grouping genes by the '", opt$name, "' attribute...\n", sep="")
