...
Comments:
SAM file is expected to be generated with a recent (>= 0.1.4) version of segemehl. In particular, the presence of the MD tag is required. Moreover, only FLAG values of 0 and 16 are currently supported (reads with other FLAG values are discarded).
...
### Type: Specialized
	...
	#---> BODY <---#
		...
		while (<IN>) {
			...
			#---> Skip reads with unsupported FLAG value <---#
			warn "[WARNING] Unsupported FLAG value in line ${.}. Entry skipped.\n" and next unless $FLAG == 0 || ($FLAG == 16 && $no_strand_info);	# Only reads with no flags set are supported; however, if --no-strand-info is set, reads are supported for which only(!) the "reverse complement flag" is set (value 16)
		...
		for my $idx ( 2 .. (scalar @trx_entry - 1) ) {
			...
			if ( $start_trx <= $trx_entry[$idx]->[3]) {
				...
				if ( $end_trx <= $trx_entry[$idx]->[3] ) {
					# Return FALSE (i.e. 0) if whole alignment is contained in 1 exon and --inlcude-monoexonic is not set
					return 0 unless $monoexonic;
