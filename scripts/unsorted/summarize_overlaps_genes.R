...
### Usage: Rscript ./summarize_overlaps_genes.R /path/to/BAM/files/ ./annotation_file.bed hg19 Union ./overlaps.R 
...
exons <- import.bed(args[2], genome=args[3], asRangedData=FALSE)
# Group by gene
genes <- split(exons, exons$name)
###

###. C. OVERLAPS
overlaps <- summarizeOverlaps(genes, read_files_ls, mode = args[4], ignore.strand=TRUE)
