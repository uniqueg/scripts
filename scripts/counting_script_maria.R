...
option_list <- list(
    ...
    make_option(
        ...
        action="store",
        type="character",
        default=file.path(getwd(), "overlaps"),
        help="Output file prefix. If not provided, the prefix 'overlaps', in the current working directory, is used.",
        metavar="dir"
    ),
    make_option(
        "--paired",
...
if ( is.proper.pair ) is.paired <- TRUE
...
if ( verb ) cat("Importing BAM file...\n", sep="'")
if ( is.null(paired) ) {
    param <- NULL
} else if ( paired == 0 ) {
    param <- ScanBamParam(
...
if ( is.null(chr.sizes.file) ) {
    ...
    seq.info <- Seqinfo(df[, 1], seqlengths=df[, 2])
}
