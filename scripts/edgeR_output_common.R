...
### B. Prepare output folder and output filename fragments
# Generate output folder name for current comparison
out_path <- paste(out_folder, "/common/", sep="")
# Create output folder
dir.create(out_path, mode = "0775")
# Generate output file path/prefix for current comparison
prefix <- paste(out_path, out_prefix, "common", sep="")
###

files <- dir(path=objects$row[6], pattern=glob2rx(objects$row[7]), recursive=TRUE, full.names=TRUE)
