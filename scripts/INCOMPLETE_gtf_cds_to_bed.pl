...
### Output: Returns a tab-separated file of the following format: transcript id TAB start TAB stop
...
### Type: Generic
	...
	while (<GTF>) {
		...
		push @exons, \%fields if $type eq "exon";
		push @start, \%fields if $type eq "start_codon";
		push @stop, \%fields if $type eq "stop_codon";				
