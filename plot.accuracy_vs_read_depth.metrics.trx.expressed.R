...
col_select <- 4
select <- c("BitSeq", "CEM", "Cufflinks", "eXpress", "IsoEM", "MMSEQ", "RSEM", "rSeq", "Sailfish", "Scripture", "TIGAR2")
prefix_values <- file.path(root, "plots", "sim.trx.metrics.scaled_values.accuracy_vs_read_depth.expressed")
prefix_ranks <- file.path(root, "plots", "sim.trx.metrics.ranks.accuracy_vs_read_depth.expressed")
...
values_ls <- lapply(read_depths, function(read_depth) {
        #<--- LOAD METRICS --->#
        spearman <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".trx.spearman.tab")), sep="\t")[ , col_select, drop=FALSE]
        pearson <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".trx.pearson.tab")), sep="\t")[ , col_select, drop=FALSE]
        rmse <- read.table(file.path(infolder, paste0("reference_comparison_Ground_truth.sim_", read_depth, ".trx.rmse.tab")), sep="\t")[ , col_select, drop=FALSE]
	#<--- MERGE --->#
